...
async function handleCallbackQuery({
  ...
  const adminName = admin.first_name || admin.username || "администратор";
  ...
  if (data === "booking_done") {
    ...
    await answerCallbackQuery(token, callbackId, "Отметили: бронь передана ✅");
    return;
  }

  if (data === "booking_done_already") {
    await answerCallbackQuery(token, callbackId, "Эта бронь уже отмечена ✅");
    return;
  }
  ...
  if (data === "duck_already_checked") {
    await answerCallbackQuery(token, callbackId, "Эта уточка уже проверена");
    return;
  }
...
async function handleDuckPhoto({
  ...
  if (!adminChatId) {
    await sendTelegramMessage(
      token,
      message.chat.id,
      "Фото получили, но не настроена группа админов для проверки."
    );
    return;
  }
  ...
  const phone =
    guestResult.ok && guestResult.guest
...
async function saveGuestToSupabase({ supabaseUrl, supabaseKey, guest }) {
  const cleanSupabaseUrl = cleanSupabaseUrlFunc(supabaseUrl);
...
function cleanSupabaseUrlFunc(supabaseUrl) {
  return String(supabaseUrl || "")
    .replace(/\/$/, "")
    .replace(/\/rest\/v1$/, "");
}
